...
async function createClicksTable() {
  ...
  await pool.query(`
    CREATE TABLE clicks (
      ...
      aff_sid VARCHAR(100) UNIQUE,
      click_type click_type_enum NOT NULL,
      original_url TEXT,
      affiliate_url TEXT,
      utm_source VARCHAR(100) DEFAULT 'chatchiu',
      utm_medium VARCHAR(100),
      utm_campaign VARCHAR(100) DEFAULT 'cashback',
...
async function seedMerchants() {
  ...
  for (const merchant of merchants) {
    await pool.query(
      `INSERT INTO merchants (id, name, logo_url, campaign_id, commission_rate, policy_note, is_active, deep_link_base)
