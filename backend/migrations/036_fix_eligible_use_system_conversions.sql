-- Migration: Fix get_eligible_conversions_for_waiting_list() to use system_conversions
-- Description: Switch from conversions (all AT data) to system_conversions (ChatChiu synced data)
-- Date: 2026-01-03
-- Issue: Loading all AccessTrade conversions instead of only ChatChiu system conversions

-- Drop old function
DROP FUNCTION IF EXISTS get_eligible_conversions_for_waiting_list() CASCADE;

-- Recreate with system_conversions table and exact type matching
...
RETURNS TABLE (
  ...
  order_code VARCHAR(100),          -- Changed from VARCHAR(255) to match system_conversions
  order_amount NUMERIC(15,2),
  commission NUMERIC(15,2),
  cashback_amount NUMERIC(15,2),
  order_time TIMESTAMP,              -- Changed from TIMESTAMPTZ to TIMESTAMP
  approval_time TIMESTAMP,           -- Changed from TIMESTAMPTZ to TIMESTAMP
  eligible_date DATE,
  approval_month DATE,
...
COMMENT ON FUNCTION get_eligible_conversions_for_waiting_list() IS
  'Returns approved conversions from ChatChiu system (system_conversions) eligible for reconciliation (approval_time + 15 days <= NOW), excluding orders already in reconciliations or waiting list';

SELECT 'Migration 036: Fixed eligible conversions to use system_conversions instead of conversions' AS status;
